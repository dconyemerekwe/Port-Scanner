...
for host in scanner.all_hosts(): # Outer loop iterates list of hosts, returned by all_hosts method of scanner
    print("Host: %s (%s)" % (host, scanner[host].hostname()))
    print("State: %s" % scanner[host].state())
    for protocol in scanner[host].all_protocols(): # Inner loop iterates through list of protocols, used by that indivdual host, returns a list of protocols (TCP UDP dicovered during host scan)
